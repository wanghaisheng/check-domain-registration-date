...
def process_row(row, index, db_path):
    ...
    web_title = WebTitle(domain, 1)
    web_title.start()

    data["title"] = web_title.result[domain]
    data["status"] = "1"
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        try:
            ...
            cursor.execute(
                """
                    INSERT INTO destinations (id,destination, category, traffic_share, visits, changes, channel, type, rdap, whois, whodap,status,domain_title)
                    VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)
                    """,
                (
                    data["id"],
